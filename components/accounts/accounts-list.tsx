...
function categorySearchBlob(a: AccountListItem) {
  ...
  return cats
    .map((bc) => (bc.category?.name ?? "").toLowerCase())
    .join(" ");
}
...
function AccountCatalogCard({ account }: { account: AccountListItem }) {
  ...
  return (
    <Link
      ...
      className="group block w-full min-w-0 rounded-xl border-2 border-slate-900 bg-linear-to-br from-violet-50 via-white to-fuchsia-50 p-4 text-left shadow-[4px_4px_0px_0px_#0f172a] transition hover:-translate-y-0.5 hover:from-violet-100 hover:to-fuchsia-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-900"
    >
      <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">
        Borrower
      </p>
      <p className="mt-0.5 truncate text-lg font-black uppercase text-slate-900">
        {name}
      </p>
      {categories.length > 0 ? (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {categories.map(({ category }) => {
            ...
            return (
              <span
                key={id}
                className="inline-flex items-center rounded-md border border-slate-900 px-2 py-0.5 text-[10px] font-bold uppercase shadow-[2px_2px_0px_0px_#1e293b]"
                style={{
      ...
      {prog.total > 0 ? (
        <div className="mt-3">
          <div className="flex justify-between text-[10px] font-black uppercase tracking-wide text-slate-600">
            <span>Schedule progress</span>
            <span className="tabular-nums text-slate-900">{prog.pct}%</span>
          </div>
          <div
            className="mt-1 h-2 overflow-hidden rounded-md border-2 border-slate-900 bg-white shadow-[1px_1px_0px_0px_#0f172a]"
            role="progressbar"
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        <div className="rounded-lg border-2 border-slate-900 bg-white/80 px-2 py-1.5">
        ...
        <div className="rounded-lg border-2 border-slate-900 bg-white/80 px-2 py-1.5">
          ...
          <p className="font-black tabular-nums text-slate-900">{rate}%</p>
        </div>
        <div className="col-span-2 rounded-lg border-2 border-slate-900 bg-sky-100/70 px-2 py-1.5">
          <p className="font-bold uppercase tracking-wide text-slate-500">
            Payment frequency
          </p>
          <p className="font-black capitalize text-slate-900">{freq}</p>
        </div>
      </div>
    </Link>
  );
...
export default function AccountsList({ accounts }: { accounts: AccountListItem[] }) {
  ...
  const hasActiveFilters =
    frequencySet.size > 0 || interestSet.size > 0 || normalizedSearch.length > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border-2 border-slate-900/90 bg-slate-50/50 px-4 py-3 shadow-[2px_2px_0px_0px_rgb(15_23_42/0.85)]">
        ...
        <p className="mt-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
          ...
          {distinctFrequencies.map((f) => (
            <span key={f} className="ml-2 normal-case">
              <span className="capitalize text-slate-800">{f}</span>{" "}
      ...
      <div className="space-y-4">
              ...
              {distinctFrequencies.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-lg border-2 px-3 py-1.5 text-xs font-black uppercase tracking-wide transition ${selected
                        ? "border-slate-900 bg-slate-900 text-white shadow-[2px_2px_0px_0px_#0f172a]"
                        : "border-slate-900/25 bg-white text-slate-800 shadow-[1px_1px_0px_0px_rgb(15_23_42/0.12)] hover:border-slate-900/50"
                      }`}
                  >
              ...
              {distinctInterests.map((r) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-lg border-2 px-3 py-1.5 text-xs font-black tabular-nums transition ${selected
                        ? "border-slate-900 bg-slate-900 text-white shadow-[2px_2px_0px_0px_#0f172a]"
                        : "border-slate-900/25 bg-white text-slate-800 shadow-[1px_1px_0px_0px_rgb(15_23_42/0.12)] hover:border-slate-900/50"
                      }`}
                  >
        ...
        {hasActiveFilters ? (
          <button
            ...
            className="rounded-lg border-2 border-rose-800/35 bg-rose-50 px-3 py-2 text-xs font-black uppercase tracking-wide text-rose-900 shadow-[1px_1px_0px_0px_rgb(190_18_60/0.25)] transition hover:bg-rose-100/90"
          >
