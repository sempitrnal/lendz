...
export default function NextCollectionPanel({
  ...
  const [navigatingBorrowerId, setNavigatingBorrowerId] = useState<
    string | null
  >(null);
  ...
  const groupedByCategory = useMemo(() => {
    const groups = new Map<
      string,
      { color: string | null; entries: NextCollectionEntry[] }
    >();
    ...
    return Array.from(groups.entries()).map(
      ([category, { color, entries: groupEntries }]) => ({
        category,
        color,
        entries: groupEntries,
        total: groupEntries.reduce((sum, e) => sum + e.amount, 0),
        accountCount: groupEntries.reduce(
  ...
  return (
    <article
      className="min-w-0 rounded-xl border-2 border-slate-900 bg-linear-to-br
        from-emerald-50 via-white to-lime-100 p-4
        shadow-[4px_4px_0px_0px_#0f172a] sm:p-5"
    >
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-base font-black lowercase text-slate-600">
          next collection
        </h2>
        {entries.length > 0 ? (
          <button
            type="button"
            disabled={isMarkingAll || navigatingBorrowerId !== null}
            onClick={() => {
              void markAllNextPaid();
            }}
            className="rounded-md border-2 border-slate-900 bg-emerald-200 px-4
              py-1 text-[12px] font-black uppercase text-slate-600
      ...
      {entries.length === 0 ? (
        <div
          className="rounded-lg border-2 border-dashed border-slate-400
        ...
        <div className="space-y-4">
            ...
            <div key={group.category}>
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className="rounded-md border border-slate-900/20 bg-white
                      px-1.5 py-0.5 text-[10px] font-bold tabular-nums
                      text-slate-600"
                  >
                    {group.accountCount} account
                ...
                {group.entries.map((entry) => {
                  ...
                  return (
                    <li
                      ...
                      tabIndex={
                        entry.borrowerId && navigatingBorrowerId === null
                          ? 0
                          : undefined
                      }
                      aria-busy={isOpening}
                    ...
                    >
                      <div
                        className="flex items-start mb-5 justify-between gap-2"
                      >
                        <p
                          className="min-w-0 flex-1 text-xl font-black uppercase
                            text-slate-600"
                        >
                          {entry.name}
                        </p>
                        {entry.schedules.length > 1 ? (
                          <span
                            className="shrink-0 rounded-md border
                              border-slate-900/20 bg-white px-1.5 py-0.5
          ...
          <div
            className="mt-3 flex items-center justify-between rounded-lg
              border-2 border-slate-900 bg-slate-900 px-3 py-2"
          >
            <span
            ...
            <span className="text-xs font-black tabular-nums text-white">
              {entries.reduce((sum, e) => sum + e.schedules.length, 0)} account
              {entries.reduce((sum, e) => sum + e.schedules.length, 0) === 1
                ? ""
                : "s"}{" "}
