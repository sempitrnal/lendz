...
export default function CategoryDetailStrip({
  ...
  return (
    <div className="sticky top-10 sm:top-16 z-30 -mx-4 bg-white/95 backdrop-blur relative">
      ...
      >
        <div className="min-w-0 text-left">
          ...
          <div className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-slate-500">
            <span>
              <strong className="text-slate-700">{borrowerCount}</strong> borrowers
            </span>
            <span>
              <strong className="text-slate-700">{borrowersWithAccountsCount}</strong> with accounts
            </span>
            {nextCollectionDate && (
              <span>
                Next: <strong className="text-slate-700">{new Date(nextCollectionDate).toLocaleDateString()}</strong>
              </span>
            )}
          </div>
        </div>
      ...
      <div
        className={`absolute left-0 right-0 top-full z-30 grid overflow-hidden bg-white/95 backdrop-blur shadow-[0_4px_0_0_#0f172a] transition-[grid-template-rows] duration-300 ease-in-out ${open ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
      >
        <div className="overflow-hidden">
          <div className="px-4 pb-3 pt-1">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 text-[11px]">
              {[
                { label: "Borrowers", value: borrowerCount, bg: "bg-sky-100" },
                { label: "With accounts", value: borrowersWithAccountsCount, bg: "bg-emerald-100" },
                { label: "To collect", value: fmtMoney(moneyToCollect), bg: "bg-amber-100", isText: true },
                {
