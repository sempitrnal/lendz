        ...
        {upcoming.map((evt) => {
          ...
          return (
            <Link
              key={evt.id}
              href={`/calendar`}
              className={`flex items-center justify-between border-b border-slate-200 px-4 py-3 transition-colors hover:bg-slate-50 last:border-b-0 ${
                isPast ? "opacity-70" : ""
              }`}
            >
              <div className="flex flex-col gap-0.5">
                ...
                {evt.amount > 0 ? (
                  ...
                  <span className="text-xs text-slate-500">
                    ₱{Number(account.principal_amount ?? 0).toLocaleString()} — {account.status}
                  </span>
                ) : null}
                {evt.note ? (
